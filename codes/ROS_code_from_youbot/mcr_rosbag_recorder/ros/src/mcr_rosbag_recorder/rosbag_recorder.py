...
class RosbagRecorder(object):
    ...
    def init_state(self):
        ...
        """
        if self.event is not None and self.event.startswith('e_start'):
            return 'RUNNING'
        else:
            return 'INIT'
    ...
    def start_recording(self):
        ...
        """
        if int(self.event[8:]) == 1:
            postfix = 'straight'
        elif int(self.event[8:]) == 2:
            postfix = 'left'
        elif int(self.event[8:]) == 3:
            postfix = 'right'
        ...
        filename = subfilename + '_' + postfix +  '.bag'
